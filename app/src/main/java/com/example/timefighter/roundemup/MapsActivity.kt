        ...
        ref.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                dataSnapshot.children.forEach {
                    val address = it.child("address").getValue().toString()
                    val score = it.child("highScore").getValue().toString()
                    mMap.addMarker((MarkerOptions().position(getLatLng(address)).title(it.key).snippet(score)))
                }
            }
